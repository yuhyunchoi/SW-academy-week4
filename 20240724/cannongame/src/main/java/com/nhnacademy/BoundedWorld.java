            ...
            if (ball instanceof MovableBall) {
                for (int j = 0; j < getBallCount(); j++) {
                    ...
                    if ((ball != otherBall) && (ball.getBounds().intersects(otherBall.getBounds()))) {
                        Rectangle intersection = ball.getBounds().intersection(otherBall.getBounds());

                        if (ball.getBounds().getWidth() < otherBall.getBounds().getWidth()) {
                            if (intersection.getWidth() < ball.getBounds().getWidth()) {
                                ((MovableBall) ball).setDX(-((MovableBall) ball).getDX());
                            }

                            if (intersection.getHeight() < ball.getBounds().getHeight()) {
                                ((MovableBall) ball).setDY(-((MovableBall) ball).getDY());
                            }

                        } else {
                            if (intersection.getWidth() < otherBall.getBounds().getWidth()) {
                                ((MovableBall) ball).setDX(-((MovableBall) ball).getDX());
                            }

                            if (intersection.getHeight() < otherBall.getBounds().getHeight()) {
                                ((MovableBall) ball).setDY(-((MovableBall) ball).getDY());
                            }
